...
export function AcademicChecklist({
  disciplineId,
  lessons,
  onProgressUpdate,
}: AcademicChecklistProps) {
      ...
      <div className="p-5 space-y-3">
            ...
            <div className="flex-1">
              <p
                className={cn(
                  "text-sm font-bold",
                  completedItems[item.id]
                    ? "text-[#27AE60] line-through opacity-70"
                    : "text-[#0A3D52]",
                )}
              >
                {item.label}
        ...
        {activeLesson?.items.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-xs font-bold text-[#0A3D52]/40 uppercase">
              Nenhuma atividade registrada para esta aula.
            </p>
          </div>
        )}
      </div>
