...
/** Valida UMA questão vinda da IA. Retorna null se inválida. */
export function validarQuestao(raw: unknown): QuestaoGerada | null {
  ...
  if (typeof q.enunciado !== "string" || q.enunciado.trim().length < 10) return null;
  if (!Array.isArray(q.alternativas)) return null;
  const alts = (q.alternativas as unknown[]).map(String).filter((a) => a.trim().length > 0);
  if (alts.length < 2) return null;
  const idx =
    typeof q.resposta_correta === "number"
      ? q.resposta_correta
      : parseInt(String(q.resposta_correta ?? ""), 10);
  if (Number.isNaN(idx) || idx < 0 || idx >= alts.length) return null;
  const dif = q.dificuldade === "facil" || q.dificuldade === "dificil" ? q.dificuldade : "medio";
  return {
...
export function extrairJsonStrict(texto: string): { questoes?: unknown[] } | null {
  ...
  try {
    ...
    while ((m = re.exec(limpo)) !== null) candidatos.push(m[0]);
    // + fallback guloso
...
- Explicação objetiva em 1-2 frases.`;
  ...
  try {
    ...
    const json = extrairJsonStrict(texto);
